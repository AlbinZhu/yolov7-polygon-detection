...
from utils.general import bbox_iou, bbox_alpha_iou, box_iou, box_giou, box_diou, box_ciou, xywh2xyxy, polygon_bbox_iou, polygon_box_iou, wh_iou, order_corners
...
class Polygon_ComputeLossOTA:
    # Compute losses
    def __init__(self, model, autobalance=False):
        super(Polygon_ComputeLossOTA, self).__init__()
        device = next(model.parameters()).device  # get model device
        h = model.hyp  # hyperparameters
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        bs, as_, gjs, gis, targets, anchors = self.build_targets(p, targets, imgs)
        pre_gen_gains = [torch.tensor(pp.shape, device=device)[[3, 2, 3, 2, 3, 2, 3, 2]] for pp in p] 
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # pbox = torch.cat((pxy, pwh), 1)  # predicted box
                pbox = ps[:, :8]
                selected_tbox = targets[i][:, 2:] * pre_gen_gains[i]
                selected_tbox -= grid.repeat(1, 4)
                iou = polygon_bbox_iou(pbox, selected_tbox, CIoU=True, device=device, ordered=True)   # iou(prediction, target)
                # lbox += (1.0 - iou).mean()  # iou loss
                ...
                lbox += (torch.max(zero, ps[:, 1]-ps[:, 5])**2).mean()/6 + (torch.max(zero, ps[:, 3]-ps[:, 5])**2).mean()/6 + \
                        ...
                        (torch.max(zero, ps[:, 0]-ps[:, 2])**2).mean()/6 + (torch.max(zero, ps[:, 6]-ps[:, 4])**2).mean()/6
                # include the values of each vertice of poligon into loss function
                lbox += nn.SmoothL1Loss(beta=0.11)(pbox, selected_tbox)
    ...
    def build_targets(self, p, targets, imgs):
        ...
        #indices, anch = self.find_positive(p, targets)
        tcl, tbox, indices, anch = self.find_3_positive(p, targets)
        #indices, anch = self.find_4_positive(p, targets)
        #indices, anch = self.find_5_positive(p, targets)
        #indices, anch = self.find_9_positive(p, targets)
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            # txyxyxyxy = this_target[:, 2:10] * imgs[batch_idx].shape[1]
            # grid = torch.stack([gi, gj], dim=1)
            txyxyxyxy = this_target[:, 2:10] * imgs[batch_idx].shape[1]
            txyxyxyxy = order_corners(txyxyxyxy)
            # txyxy = xywh2xyxy(txywh)
            ...
            for i, pi in enumerate(p):
                ...
                fg_pred = pi[b, a, gj, gi]                
                p_obj.append(fg_pred[:, 8:9])
                p_cls.append(fg_pred[:, 9:])
                ...
                # pxywh = torch.cat([pxy, pwh], dim=-1)
                # pxyxy = xywh2xyxy(pxywh)
                pxyxyxyxy = (fg_pred[:, :8] + grid[i].repeat((1, 4))) * self.stride[i]
                pxyxyxyxys.append(pxyxyxyxy)
            
            pxyxyxyxys = torch.cat(pxyxyxyxys, dim=0)
            if pxyxyxyxys.shape[0] == 0:
                continue
            p_obj = torch.cat(p_obj, dim=0)
            p_cls = torch.cat(p_cls, dim=0)
            from_which_layer = torch.cat(from_which_layer, dim=0)
            ...
            pair_wise_iou = polygon_box_iou(txyxyxyxy, pxyxyxyxys, device=device, ordered=True)
    ...
    def find_3_positive(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            if nt:
                ...
                r = wh / anchors[:, None]  # wh ratio
                j = torch.max(r, 1. / r).max(2)[0] < self.hyp['anchor_t']  # compare
                # j = wh_iou(anchors, wh[0]) > self.hyp['iou_t']  # iou(3,n)=wh_iou(anchors(3,2), gwh(n,2))
                t = t[j]  # filter

                # Offsets
                center_x = t[:, 2:10:2].mean(dim=-1)[:, None]
                center_y = t[:, 3:10:2].mean(dim=-1)[:, None]
                # x = (torch.max(t[:, 2:-1:2], -1)[0] + torch.min(t[:, 2:-1:2], -1)[0]) / 2
                # y = (torch.max(t[:, 3:-1:2], -1)[0] + torch.min(t[:, 3:-1:2], -1)[0]) / 2
                gxy = torch.cat((center_x, center_y), dim=-1)  # grid xy
                gxi = gain[[2, 3]] - gxy  # inverse
                j, k = ((gxy % 1. < g) & (gxy > 1.)).T
                l, m = ((gxi % 1. < g) & (gxi > 1.)).T
