...
from utils.general import coco80_to_coco91_class, check_dataset, check_file, check_img_size, check_requirements, \
    box_iou, non_max_suppression, polygon_non_max_suppression, scale_coords, polygon_scale_coords, xyxy2xywh, xywh2xyxy, set_logging, increment_path, colorstr, polygon_box_iou
...
def test(data,
    ...
    if not training:
        ...
        dataloader = create_dataloader(data[task], imgsz, batch_size, gs, opt, pad=0.5, rect=True,
                                       prefix=colorstr(f'{task}: '), polygon=True)[0]
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            t = time_synchronized()
            out = polygon_non_max_suppression(out, conf_thres=conf_thres, iou_thres=iou_thres, labels=lb, multi_label=True)
        ...
        for si, pred in enumerate(out):
            ...
            if nl:
                ...
                for cls in torch.unique(tcls_tensor):
                    ...
                    pi = (cls == pred[:, 9]).nonzero(as_tuple=False).view(-1)  # target indices
